...
module barbut::single_player_barbut{
    ...
    // Counter library
    use barbut::counter_nft::{Self, Counter};

    // HouseData library
    use barbut::house_data::{Self as hd, HouseData};
    ...
    /// Function used to create a new game. The player must provide a Counter NFT.
    ...
    public fun finish_game(game_id: ID, bls_sig: vector<u8>, house_data: &mut HouseData, ctx: &mut TxContext) {
        ...
        let second_byte = *vector::borrow(&hashed_beacon, 1);

        let player_dice = (first_byte%6)+1;
        let house_dice = (second_byte%6)+1;

        let player_won = house_dice < player_dice;
    ...
    /// The player must provide a Counter NFT.
    /// Stake is taken from the player's coin and added to the game's stake. 
    /// The house's stake is also added to the game's stake.
    fun internal_start_game(counter: &mut Counter, coin: Coin<SUI>, house_data: &mut HouseData, fee_bp: u16, ctx: &mut TxContext): (ID, Game) {
